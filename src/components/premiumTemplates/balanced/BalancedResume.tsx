...
const BalancedCustomSection = ({ title, items }: { title: string; items: string[] }) => {
...
const BalancedPortfolioSection = ({ profile }: { profile: any }) => {
...
export const BalancedResume = ({ profile, onRegenerate }: { profile: any; onRegenerate?: () => void }) => {
  ...
  const renderedSections = useMemo(() => {
    ...
    const summarySec = <BalancedProfile key="summary" profile={typedProfile} />;
    const competenciesSec = <BalancedCompetencies key="competencies" profile={typedProfile} />;
    const experienceSec = <BalancedExperience key="experience" profile={typedProfile} />;
    const projectsSec = <BalancedProjects key="projects" profile={typedProfile} />;
    const educationSec = <BalancedEducation key="education" profile={typedProfile} />;
    const certificationsSec = <BalancedCertifications key="certifications" profile={typedProfile} />;
    const achievementsSec = <BalancedAchievements key="achievements" profile={typedProfile} />;

    // Custom sections
    const practiceAreasSec = (
      <BalancedCustomSection 
        key="practiceAreas" 
    ...
    const portfolioSec = <BalancedPortfolioSection key="portfolio" profile={typedProfile} />;
    const campaignsSec = (
      <BalancedCustomSection 
        key="campaigns" 
    ...
    if (blueprint) {
      ...
      dynamicSlots.forEach(slot => {
        if (slot.id === 'experience') list.push(<BalancedExperience key={`slot-${slot.id}`} profile={typedProfile} title={slot.label} />);
        if (slot.id === 'projects') list.push(<BalancedProjects key={`slot-${slot.id}`} profile={typedProfile} title={slot.label} />);
        if (slot.id === 'skills') list.push(<BalancedCompetencies key={`slot-${slot.id}`} profile={typedProfile} title={slot.label} />);
        if (slot.id === 'education') list.push(<BalancedEducation key={`slot-${slot.id}`} profile={typedProfile} title={slot.label} />);
        if (slot.id === 'certifications') list.push(<BalancedCertifications key={`slot-${slot.id}`} profile={typedProfile} title={slot.label} />);
        if (slot.id === 'achievements') list.push(<BalancedAchievements key={`slot-${slot.id}`} profile={typedProfile} />); 
      });
      
    } else {
      // Default fallback
      list.push(competenciesSec);
    ...
    return list;
  }, [typedProfile, isDesigner, isLawyer, isMarketing]);
  ...
  return (
    <div className="font-sans text-gray-900 bg-white balanced-resume-container">
