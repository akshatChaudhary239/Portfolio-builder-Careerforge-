...
export default function ProjectsTechnical({ profile }: BaseSectionProps) {
  ...
  return (
    <motion.div key="projects" id="projects" {...fadeInUp} className="space-y-8 scroll-mt-24">
      <div className="flex items-end justify-between border-b border-[rgba(var(--color-primary-rgb),0.1)] pb-4">
        <div>
          <h3 className="text-[10px] font-bold uppercase tracking-widest text-[var(--color-primary)] mb-2">Featured Projects</h3>
          <h2 className="text-3xl font-bold text-[var(--color-text)] tracking-tight">Things I've Built</h2>
        </div>
        <a href="#projects" className="hidden sm:flex items-center gap-2 text-sm text-[var(--color-muted)] hover:text-[var(--color-text)] transition-colors">
        ...
        {profile.projects.map((proj: any, idx: number) => (
          <div key={idx} className="bg-[var(--color-surface)] border border-[rgba(var(--color-primary-rgb),0.1)] rounded-[var(--radius-base)] overflow-hidden flex flex-col h-full hover:-translate-y-1 hover:border-[var(--color-primary)] hover:shadow-[0_10px_40px_rgba(var(--color-primary-rgb),0.1)] transition-all duration-300 group">
            ...
            <div className="p-6 md:p-8 flex flex-col flex-grow">
              <h4 className="text-xl font-bold text-[var(--color-text)] tracking-tight mb-2">{proj.name || (proj as any).title}</h4>
              <p className="text-[var(--color-muted)] text-sm leading-relaxed mb-6 flex-grow font-[var(--font-body)]">{proj.description}</p>
              ...
              {proj.achievements && proj.achievements.length > 0 && (
                <ul className="space-y-2 mt-4 flex-grow">
                  {proj.achievements.map((bullet: any, bIdx: number) => (
                    <li key={bIdx} className="relative pl-4 before:content-['•'] before:absolute before:left-0 before:text-[var(--color-primary)] text-[13px] text-[var(--color-muted)] font-serif leading-relaxed">{bullet}</li>
                  ))}
                </ul>
              )}

              <div className="flex items-center gap-4 pt-4 border-t border-[rgba(var(--color-primary-rgb),0.1)]">
                {proj.liveUrl && (
                  <a href={proj.liveUrl} target="_blank" rel="noopener noreferrer" className="text-sm font-medium text-[var(--color-primary)] hover:opacity-80 transition-opacity flex items-center gap-1.5">
                    View Project <ExternalLink size={14} />
                  </a>
                )}
                {proj.githubUrl && (
                  <a href={proj.githubUrl} target="_blank" rel="noopener noreferrer" className="text-[var(--color-muted)] hover:text-[var(--color-text)] transition-colors">
